...
(!empty(isset($_GET['search_note'])))
? $search_note = $_GET['search_note'] : $search_note = "";
...
? $search_buyer = $_GET['search_buyer'] : $search_buyer = "";
        ...
        $query = "SELECT D.`NoteID`, D.`NoteTitle` AS Title, D.`NoteCategory` AS Category, BU.`UserID` AS BuyerID, BU.`FirstName` AS BuFName, BU.`LastName` AS BuLName, SL.`UserID` AS SellerID, SL.`FirstName` AS SlFName, SL.`LastName` AS SlLName, D.`IsPaid` As IsPaid, D.`PurchasedPrice` AS SellPrice, D.`AttachmentDownloadedDate` AS DownloadDate FROM `user` AS SL
        ...
        if($search_note != ""){
            $query .= " AND D.`NoteID` = '$search_note' ";
        }
        if($search_seller != ""){
